...
/**
 * CheapAAC is a CheapSoundFile implementation for AAC (Advanced Audio Codec)
 * encoded sound files. It supports files with an MP4 header, including
 * unencrypted files encoded by Apple iTunes, and also files with a more basic
 * ADTS header.
 */
public class CheapAAC extends CheapSoundFile {
	...
	public static final int[] kRequiredAtoms = { kDINF, kHDLR, kMDHD, kMDIA,
			kMINF, kMOOV, kMVHD, kSMHD, kSTBL, kSTSD, kSTSZ, kSTTS, kTKHD,
			kTRAK, };

	public static final int[] kSaveDataAtoms = { kDINF, kHDLR, kMDHD, kMVHD,
			kSMHD, kTKHD, kSTSD, };
	...
	public void ReadFile(File inputFile) throws java.io.FileNotFoundException,
		...
		if (header[0] == 0 && header[4] == 'f' && header[5] == 't'
	...
	private void parseMp4(InputStream stream, int maxLen)
		...
		while (maxLen > 8) {
			...
			int atomLen = ((0xff & atomHeader[0]) << 24)
			...
			if (atomLen > maxLen)
				atomLen = maxLen;
			int atomType = ((0xff & atomHeader[4]) << 24)
			...
			if (atomType == kMOOV || atomType == kTRAK || atomType == kMDIA
					|| atomType == kMINF || atomType == kSTBL) {
			...
			if (skipLen < 0) {
				throw new java.io.IOException("Went over by " + (-skipLen)
						+ " bytes");
			}
	...
	void parseStts(InputStream stream, int maxLen) throws java.io.IOException {
		...
		mOffset += 16;
		mSamplesPerFrame = ((0xff & sttsData[12]) << 24)
				| ((0xff & sttsData[13]) << 16) | ((0xff & sttsData[14]) << 8)
				| ((0xff & sttsData[15]));
	...
	void parseStsz(InputStream stream, int maxLen) throws java.io.IOException {
		...
		mOffset += 12;
		mNumFrames = ((0xff & stszHeader[8]) << 24)
	...
	void parseMdat(InputStream stream, int maxLen) throws java.io.IOException {
		...
		for (int i = 0; i < mNumFrames; i++) {
			...
			if (mProgressListener != null) {
				boolean keepGoing = mProgressListener.reportProgress(mOffset
						* 1.0 / mFileSize);
				if (!keepGoing) {
					break;
				}
	...
	void readFrameAndComputeGain(InputStream stream, int frameIndex)
		...
		case 1: // ID_CPE: stereo
			...
			for (int b = 0; b < 8; b++) {
				...
				/*
				 * System.out.println("Bit " + (b + startBit) + " " + "b0 " + b0
				 * + " " + "b1 " + b1 + " " + "add " + add);
				 */
	...
	public void WriteFile(File outputFile, int startFrame, int numFrames)
		...
		SetAtomData(kFTYP, new byte[] { 'M', '4', 'A', ' ', 0, 0, 0, 0, 'M',
				'4', 'A', ' ', 'm', 'p', '4', '2', 'i', 's', 'o', 'm', 0, 0, 0,
				0 });
		...
		SetAtomData(kSTSC, new byte[] { 0,
				...
				(byte) ((numFrames >> 24) & 0xff),
				(byte) ((numFrames >> 16) & 0xff),
				(byte) ((numFrames >> 8) & 0xff), (byte) (numFrames & 0xff), 0,
				0, 0, 1 // Smaple desc index
				});
		...
		int mdatOffset = 144 + 4 * numFrames + mAtomMap.get(kSTSD).len
				+ mAtomMap.get(kSTSC).len + mAtomMap.get(kMVHD).len
				+ mAtomMap.get(kTKHD).len + mAtomMap.get(kMDHD).len
				+ mAtomMap.get(kHDLR).len + mAtomMap.get(kSMHD).len
		...
		mAtomMap.get(kSTBL).len = 8 + mAtomMap.get(kSTSD).len
				+ mAtomMap.get(kSTTS).len + mAtomMap.get(kSTSC).len
				+ mAtomMap.get(kSTSZ).len + mAtomMap.get(kSTCO).len;

		mAtomMap.get(kMINF).len = 8 + mAtomMap.get(kDINF).len
				+ mAtomMap.get(kSMHD).len + mAtomMap.get(kSTBL).len;

		mAtomMap.get(kMDIA).len = 8 + mAtomMap.get(kMDHD).len
				+ mAtomMap.get(kHDLR).len + mAtomMap.get(kMINF).len;

		mAtomMap.get(kTRAK).len = 8 + mAtomMap.get(kTKHD).len
				+ mAtomMap.get(kMDIA).len;

		mAtomMap.get(kMOOV).len = 8 + mAtomMap.get(kMVHD).len
	 ...
	 * File f = new File(""); CheapAAC c = new CheapAAC(); c.ReadFile(f);
